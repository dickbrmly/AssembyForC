...
extern "C" int readHexNum(int *);
extern "C" void bucketSort(int[], int);
...
//  Loops to read numbers from user.
	...
	while (!endOfNumbers && len<MAXLENGTH) {
		...
		if(readX)
			printf ("Enter X Value (hex): " );
		else
			printf ("Enter Y Value (hex): " );
		fflush(stdout);
		status = readHexNum(&newNumber);
...
//  Ensure some numbers were read and, if so, display results.
	...
	if (len <= 2) {
		...
		bucketSort(xList, len);
		bucketSort(yList, len);
